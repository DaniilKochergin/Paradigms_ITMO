...
let expression = (function () {
    ...
    let Sumexp = operatorFactory(
        'sumexp',
        function () {
            return Array.from(arguments).reduce((sum, cur) => sum + Math.exp(cur), 0);
        },
        function () {
            return this;
        }
    ...
    let Softmax = operatorFactory(
        'softmax',
        function () {
            let sumofexp = new Sumexp(arguments);
            return Array.from(arguments).reduce((sum, cur) => sum + Math.exp(cur) / sumofexp, 0);
        },
        function () {
            return null;
        }
    ...
    let parsePostfix = function (str) {
        ...
        let parseExpression = function () {
            if (tokenizer.token === '(') {
                ...
                let op = operations[tokenizer.token];
